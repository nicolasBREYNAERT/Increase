	...
	<tbody>
		{% for tache in taches %}
			<tr id="{{tache.getId()}} " class="{{tache.getId()}} bg-success tache" style="cursor:pointer">
					<td class=""><b><span id="libelle">{{ tache.getLibelle() }}</span></b> [{{ tache.getAvancement() }}]</td>
		...
		{% endfor %}
			<tr>
				<td colspan="4">
					...
					{% for tache in taches %}
						<a id="modifier-{{tache.getId()}}" class="btn btn-primary modifier" href='{{url.get("taches/modification/"~tache.getId()) }}' data-ajax="{{"taches/modification/"~tache.getId() }}" style="display:none">Modifier...</a>
